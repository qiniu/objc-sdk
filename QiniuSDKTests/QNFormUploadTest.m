...
//
//    QNUploadOption *opt = [[QNUploadOption alloc] initWithMime:@"text/plain"
//    progressHandler:nil params:@{ @"x:foo":@"bar" } checkCrc:YES
//    cancellationSignal:nil];
//    NSData *data = [@"Hello, World!" dataUsingEncoding:NSUTF8StringEncoding];
//    QNConfiguration *config = [QNConfiguration build:^(QNConfigurationBuilder
//    *builder) {
//        QNServiceAddress *s = [[QNServiceAddress alloc]
//        init:@"https://uptemp.qbox.me" ips:nil];
//        builder.zone = [[QNZone alloc] initWithUp:s upBackup:nil];
//    }];
//    QNUploadManager *upManager = [[QNUploadManager
//    alloc]initWithConfiguration:config];
//    [upManager putData:data key:@"你好" token:g_token complete:
...
- (void)testNoKey {
  ...
  [self.upManager
       putData:data
           key:nil
         token:g_token
      complete:^(QNResponseInfo *info, NSString *k, NSDictionary *resp) {
        key = k;
        testInfo = info;
        testResp = resp;
      }
  ...
  XCTAssert(testInfo.isOK, @"Pass");
  XCTAssert(testInfo.reqId, @"Pass");
  XCTAssert([@"FgoKnypncpQlV6tTVddq9EL49l4B" isEqualToString:testResp[@"key"]],
            @"Pass");
}
