...
- (void)makeFile:(NSString *)uphost complete:(QNCompleteBlock)complete;
           ...
           withConfiguration:(QNConfiguration *)config;
{
  if (self = [super init]) {
    ...
    _contexts = [[NSMutableArray alloc]
        initWithCapacity:(_size + kQNBlockSize - 1) / kQNBlockSize];
    _config = config;
...
- (void)record:(UInt32)offset {
  ...
  if (offset == 0 || _recorder == nil || key == nil ||
      [key isEqualToString:@""]) {
    return;
  }
  ...
  NSMutableDictionary *rec = [NSMutableDictionary
      dictionaryWithObjectsAndKeys:n_size, @"size", n_offset, @"offset", n_time,
                                   @"modify_time", _contexts, @"contexts", nil];

  NSError *error;
  NSData *data =
      [NSJSONSerialization dataWithJSONObject:rec
...
- (void)nextTask:(UInt32)offset
  ...
  if (offset == self.size) {
    QNCompleteBlock completionHandler =
  ...
  QNInternalProgressBlock progressBlock =
...
- (void)makeBlock:(NSString *)uphost
  ...
  NSString *url = [[NSString alloc]
      initWithFormat:@"%@/mkblk/%u", uphost, (unsigned int)blockSize];
...
- (void)putChunk:(NSString *)uphost
  ...
  NSString *url =
      [[NSString alloc] initWithFormat:@"%@/bput/%@/%u", uphost, context,
...
- (void)makeFile:(NSString *)uphost complete:(QNCompleteBlock)complete {
  NSString *mime = [[NSString alloc]
      initWithFormat:@"/mimeType/%@",
                     [QNUrlSafeBase64 encodeString:self.option.mimeType]];

  __block NSString *url = [[NSString alloc]
      initWithFormat:@"%@/mkfile/%u%@", uphost, (unsigned int)self.size, mime];

  if (self.key != nil) {
    NSString *keyStr = [[NSString alloc]
        initWithFormat:@"/key/%@", [QNUrlSafeBase64 encodeString:self.key]];
    url = [NSString stringWithFormat:@"%@%@", url, keyStr];
  }

  [self.option.params enumerateKeysAndObjectsUsingBlock:^(
                          NSString *key, NSString *obj, BOOL *stop) {
    url = [NSString stringWithFormat:@"%@/%@/%@", url, key,
                                     [QNUrlSafeBase64 encodeString:obj]];
  }];

  //添加路径
  NSString *fname = [[NSString alloc]
      initWithFormat:@"/fname/%@",
                     [QNUrlSafeBase64 encodeString:[self dealFilePath]]];
