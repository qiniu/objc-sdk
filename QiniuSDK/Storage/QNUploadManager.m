...
- (instancetype)initWithConfiguration:(QNConfiguration *)config {
  if (self = [super init]) {
    ...
    if (hasNSURLSession()) {
      _httpManager =
          [[QNSessionManager alloc] initWithProxy:config.proxy
                                          timeout:config.timeoutInterval
                                     urlConverter:config.converter
                                              dns:config.dns];
    } else {
      _httpManager =
          [[QNHttpManager alloc] initWithTimeout:config.timeoutInterval
                                    urlConverter:config.converter
                                             dns:config.dns];
    }
#else
    _httpManager = [[QNHttpManager alloc] initWithTimeout:config.timeoutInterval
                                             urlConverter:config.converter
                                                      dns:config.dns];
#endif
  }
...
+ (BOOL)checkAndNotifyError:(NSString *)key
  ...
  if (desc != nil) {
    QNAsyncRunInMain(^{
      completionHandler([QNResponseInfo responseInfoWithInvalidArgument:desc],
                        key, nil);
    });
    return YES;
  }
...
- (void)putData:(NSData *)data
  ...
  if ([QNUploadManager checkAndNotifyError:key
                                     token:token
                                     input:data
  ...
  if (t == nil) {
    QNAsyncRunInMain(^{
      completionHandler(
          [QNResponseInfo responseInfoWithInvalidToken:@"invalid token"], key,
          nil);
    });
    return;
  }
  ...
  QNUpCompletionHandler complete =
...
- (void)putFileInternal:(id<QNFileDelegate>)file
  ...
  @autoreleasepool {
    ...
    QNResumeUpload *up = [[QNResumeUpload alloc] initWithFile:file
                                                      withKey:key
                                                    withToken:t
                                        withCompletionHandler:complete
                                                   withOption:option
...
- (void)putALAsset:(ALAsset *)asset
  ...
  @autoreleasepool {
    NSError *error = nil;
    __block QNALAssetFile *file =
        [[QNALAssetFile alloc] init:asset error:&error];
    if (error) {
      QNAsyncRunInMain(^{
        QNResponseInfo *info = [QNResponseInfo responseInfoWithFileError:error];
        completionHandler(info, key, nil);
...
- (void)putPHAsset:(PHAsset *)asset
  ...
  @autoreleasepool {
    NSError *error = nil;
    __block QNPHAssetFile *file =
        [[QNPHAssetFile alloc] init:asset error:&error];
    if (error) {
      QNAsyncRunInMain(^{
        QNResponseInfo *info = [QNResponseInfo responseInfoWithFileError:error];
        completionHandler(info, key, nil);
...
- (void)putPHAssetResource:(PHAssetResource *)assetResource
  ...
  @autoreleasepool {
    NSError *error = nil;
    __block QNPHAssetResource *file =
        [[QNPHAssetResource alloc] init:assetResource error:&error];
    if (error) {
      QNAsyncRunInMain(^{
        QNResponseInfo *info = [QNResponseInfo responseInfoWithFileError:error];
        completionHandler(info, key, nil);
