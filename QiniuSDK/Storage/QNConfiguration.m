...
static QNDnsManager *initDns(QNConfigurationBuilder *builder) {
  ...
  if (d == nil) {
    ...
    id<QNResolverDelegate> r2 =
        [[QNResolver alloc] initWithAddres:@"119.29.29.29"];
    id<QNResolverDelegate> r3 =
        [[QNResolver alloc] initWithAddres:@"114.114.115.115"];
    d = [[QNDnsManager alloc] init:[NSArray arrayWithObjects:r1, r2, r3, nil]
...
- (instancetype)initWithBuilder:(QNConfigurationBuilder *)builder {
  if (self = [super init]) {
    _up = builder.zone.up;
    _upBackup =
        builder.zone.upBackup == nil ? builder.zone.up : builder.zone.upBackup;
...
+ (instancetype)createWithHost:(NSString *)up
                    backupHost:(NSString *)backup
                           ip1:(NSString *)ip1
                           ip2:(NSString *)ip2 {
...
+ (instancetype)zone0 {
  ...
  dispatch_once(&onceToken, ^{
    z0 = [QNZone createWithHost:@"upload.qiniu.com"
                     backupHost:@"up.qiniu.com"
                            ip1:@"183.136.139.10"
                            ip2:@"115.231.182.136"];
  });
  return z0;
...
+ (instancetype)zone1 {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    z1 = [QNZone createWithHost:@"upload-z1.qiniu.com"
                     backupHost:@"up-z1.qiniu.com"
                            ip1:@"106.38.227.28"
                            ip2:@"106.38.227.27"];
  });
  return z1;
