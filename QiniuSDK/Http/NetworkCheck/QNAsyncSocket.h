...
@class QNAsyncReadPacket;
@class QNAsyncWritePacket;
@class QNAsyncSocketPreBuffer;
@protocol QNAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const QNAsyncSocketException;
extern NSString *const QNAsyncSocketErrorDomain;

extern NSString *const QNAsyncSocketQueueName;
extern NSString *const QNAsyncSocketThreadName;

extern NSString *const QNAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const QNAsyncSocketUseCFStreamForTLS;
#endif
#define QNAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define QNAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define QNAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const QNAsyncSocketSSLPeerID;
extern NSString *const QNAsyncSocketSSLProtocolVersionMin;
extern NSString *const QNAsyncSocketSSLProtocolVersionMax;
extern NSString *const QNAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const QNAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const QNAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const QNAsyncSocketSSLDiffieHellmanParameters;
#endif

#define QNAsyncSocketLoggingContext 65535


typedef NS_ERROR_ENUM(QNAsyncSocketErrorDomain, QNAsyncSocketError) {
	QNAsyncSocketNoError = 0,           // Never used
	QNAsyncSocketBadConfigError,        // Invalid configuration
	QNAsyncSocketBadParamError,         // Invalid parameter was passed
	QNAsyncSocketConnectTimeoutError,   // A connect operation timed out
	QNAsyncSocketReadTimeoutError,      // A read operation timed out
	QNAsyncSocketWriteTimeoutError,     // A write operation timed out
	QNAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	QNAsyncSocketClosedError,           // The remote peer closed the connection
	QNAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface QNAsyncSocket : NSObject

/**
 * QNAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, QNAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<QNAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<QNAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq NS_DESIGNATED_INITIALIZER;

/**
 * Create QNAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<QNAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<QNAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<QNAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<QNAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<QNAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<QNAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means QNAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means QNAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, QNAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, QNAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of QNAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a QNAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a QNAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that QNAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - QNAsyncSocketManuallyEvaluateTrust
 ...
 *     So QNAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - QNAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default QNAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct QNAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, QNAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by QNAsyncSocket,
 ...
 *
 * - QNAsyncSocketSSLProtocolVersionMin
 * - QNAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - QNAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see QNAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use QNAsyncSocketSSLProtocolVersionMin & QNAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * QNAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(QNAsyncSocket *)sock
 * {
...
@protocol QNAsyncSocketDelegate <NSObject>
@optional
...
**/
- (NSTimeInterval)socket:(QNAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(QNAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
 ...
 * 
 * Note: If the QNAsyncSocket instance is deallocated while it is still connected,
 ...
 * - QNAsyncSocketManuallyEvaluateTrust == YES
 *
...
**/
- (void)socket:(QNAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
