...
#ifndef QNAsyncSocketLoggingEnabled
#define QNAsyncSocketLoggingEnabled 0
#endif

#if QNAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext QNAsyncSocketLoggingContext
...
#ifndef QNAsyncSocketLogLevel
#define QNAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = QNAsyncSocketLogLevel;
...
NSString *const QNAsyncSocketException = @"QNAsyncSocketException";
NSString *const QNAsyncSocketErrorDomain = @"QNAsyncSocketErrorDomain";

NSString *const QNAsyncSocketQueueName = @"QNAsyncSocket";
NSString *const QNAsyncSocketThreadName = @"QNAsyncSocket-CFStream";

NSString *const QNAsyncSocketManuallyEvaluateTrust = @"QNAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const QNAsyncSocketUseCFStreamForTLS = @"QNAsyncSocketUseCFStreamForTLS";
#endif
NSString *const QNAsyncSocketSSLPeerID = @"QNAsyncSocketSSLPeerID";
NSString *const QNAsyncSocketSSLProtocolVersionMin = @"QNAsyncSocketSSLProtocolVersionMin";
NSString *const QNAsyncSocketSSLProtocolVersionMax = @"QNAsyncSocketSSLProtocolVersionMax";
NSString *const QNAsyncSocketSSLSessionOptionFalseStart = @"QNAsyncSocketSSLSessionOptionFalseStart";
NSString *const QNAsyncSocketSSLSessionOptionSendOneByteRecord = @"QNAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const QNAsyncSocketSSLCipherSuites = @"QNAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const QNAsyncSocketSSLDiffieHellmanParameters = @"QNAsyncSocketSSLDiffieHellmanParameters";
#endif

enum QNAsyncSocketFlags
{
...
@interface QNAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation QNAsyncSocketPreBuffer
...
/**
 * The QNAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface QNAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(QNAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation QNAsyncReadPacket
...
/**
 * The QNAsyncWritePacket encompasses the instructions for any given write.
**/
@interface QNAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation QNAsyncWritePacket
...
/**
 * The QNAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface QNAsyncSpecialPacket : NSObject
{
  @public
...
@implementation QNAsyncSpecialPacket
...
@implementation QNAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<QNAsyncSocketDelegate> delegate;
	...
	QNAsyncReadPacket *currentRead;
	QNAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	QNAsyncSocketPreBuffer *preBuffer;
	...
	QNAsyncSocketPreBuffer *sslPreBuffer;
...
- (instancetype)initWithDelegate:(id<QNAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	return [self initWithDelegate:aDelegate delegateQueue:dq socketQueue:NULL];
}

- (instancetype)initWithDelegate:(id<QNAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([QNAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[QNAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<QNAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<QNAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  QNAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncSocketOtherError",
                                                           @"QNAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)setDelegate:(id<QNAsyncSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<QNAsyncSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id<QNAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id<QNAsyncSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id<QNAsyncSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		__strong id<QNAsyncSocketDelegate> theDelegate = delegate;
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create QNAsyncSocket instance for accepted socket
			
			QNAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncSocketConnectTimeoutError",
	                                                     @"QNAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncSocketReadMaxedOutError",
														 @"QNAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncSocketReadTimeoutError",
	                                                     @"QNAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncSocketWriteTimeoutError",
	                                                     @"QNAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"QNAsyncSocketClosedError",
	                                                     @"QNAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = @{NSLocalizedDescriptionKey : errMsg};
	
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:QNAsyncSocketErrorDomain code:QNAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the QNAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	QNAsyncReadPacket *packet = [[QNAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[QNAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[QNAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued QNAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued QNAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via QNAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		QNAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		QNAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	QNAsyncWritePacket *packet = [[QNAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[QNAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[QNAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued QNAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued QNAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a QNAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		QNAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	QNAsyncSpecialPacket *packet = [[QNAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			QNAsyncSpecialPacket *tlsPacket = (QNAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
                tlsSettings = tlsPacket->tlsSettings;
            }
			NSNumber *value = [tlsSettings objectForKey:QNAsyncSocketUseCFStreamForTLS];
...
{
	QNAsyncSocket *asyncSocket = (__bridge QNAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	NSNumber *shouldManuallyEvaluateTrust = [tlsSettings objectForKey:QNAsyncSocketManuallyEvaluateTrust];
	...
	//  3. QNAsyncSocketSSLPeerID
	//  4. QNAsyncSocketSSLProtocolVersionMin
	//  5. QNAsyncSocketSSLProtocolVersionMax
	//  6. QNAsyncSocketSSLSessionOptionFalseStart
	//  7. QNAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. QNAsyncSocketSSLCipherSuites
	//  9. QNAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:QNAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for QNAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for QNAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. QNAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:QNAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for QNAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for QNAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. QNAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:QNAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for QNAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for QNAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. QNAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:QNAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for QNAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for QNAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. QNAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:QNAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for QNAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for QNAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. QNAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:QNAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for QNAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for QNAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. QNAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:QNAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for QNAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for QNAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use QNAsyncSocketSSLProtocolVersionMin & QNAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[QNAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"QNAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[QNAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[QNAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("QNAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:QNAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(QNAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(QNAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	QNAsyncSocket *asyncSocket = (__bridge QNAsyncSocket *)pInfo;
