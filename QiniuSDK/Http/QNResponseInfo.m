...
+ (instancetype)responseInfoWithInvalidArgument:(NSString *)text {
  return [[QNResponseInfo alloc] initWithStatus:kQNInvalidArgument
                               errorDescription:text];
}

+ (instancetype)responseInfoWithInvalidToken:(NSString *)text {
  return [[QNResponseInfo alloc] initWithStatus:kQNInvalidToken
                               errorDescription:text];
}
...
+ (instancetype)responseInfoWithNetError:(NSError *)error
  ...
  return [[QNResponseInfo alloc] initWithStatus:code
                                          error:error
                                           host:host
                                       duration:duration];
}
...
+ (instancetype)responseInfoOfZeroData:(NSString *)path {
  ...
  return [[QNResponseInfo alloc] initWithStatus:kQNZeroDataSize
                               errorDescription:desc];
}

- (instancetype)initWithCancelled {
  return [self initWithStatus:kQNRequestCancelled
             errorDescription:@"cancelled by user"];
}

- (instancetype)initWithStatus:(int)status error:(NSError *)error {
...
- (instancetype)initWithStatus:(int)status errorDescription:(NSString *)text {
...
- (instancetype)init:(int)status
  ...
  if (self = [super init]) {
    ...
    if (status != 200) {
      if (body == nil) {
        _error = [[NSError alloc] initWithDomain:domain
                                            code:_statusCode
                                        userInfo:nil];
      } else {
        NSError *tmp;
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@= id: %@, ver: %@, status: %d, requestId: %@, "
                       @"xlog: %@, xvia: %@, host: %@ ip: %@ duration: %f s "
                       @"time: %llu error: %@>",
                       NSStringFromClass([self class]), _id, kQiniuVersion,
                       _statusCode, _reqId, _xlog, _xvia, _host, _serverIp,
                       _duration, _timeStamp, _error];
}
...
- (BOOL)isConnectionBroken {
  ...
  return _statusCode == kQNNetworkError ||
         (_statusCode < -1000 && _statusCode != -1003);
}

- (BOOL)needSwitchServer {
  return _statusCode == kQNNetworkError ||
         (_statusCode < -1000 && _statusCode != -1003) ||
         (_statusCode / 100 == 5 && _statusCode != 579);
}

- (BOOL)couldRetry {
  return (_statusCode >= 500 && _statusCode < 600 && _statusCode != 579) ||
         _statusCode == kQNNetworkError || _statusCode == 996 ||
         _statusCode == 406 || (_statusCode == 200 && _error != nil) ||
         _statusCode < -1000 || self.isNotQiniu;
}
