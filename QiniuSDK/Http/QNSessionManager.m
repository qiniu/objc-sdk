...
static NSURL *buildUrl(NSString *host, NSNumber *port, NSString *path) {
  ...
  NSString *p =
      [[NSString alloc] initWithFormat:@"http://%@:%@%@", host, port, path];
...
+ (QNResponseInfo *)buildResponseInfo:(NSHTTPURLResponse *)response
  ...
  if (response) {
    ...
    info = [[QNResponseInfo alloc] init:status
                              withReqId:reqId
                               withXLog:xlog
                               withXVia:xvia
                               withHost:host
...
- (void)sendRequest:(NSMutableURLRequest *)request
  ...
  if (_converter != nil) {
    ...
    ips = [_dns queryWithDomain:[[QNDomain alloc] init:domain
                                            hostsFirst:NO
                                              hasCname:YES
                                                maxTtl:1000]];
    ...
    if (ips == nil || ips.count == 0) {
      ...
      QNResponseInfo *info = [QNResponseInfo responseInfoWithNetError:error
                                                                 host:domain
                                                             duration:duration];
      NSLog(@"failure %@", info);
...
- (void)sendRequest2:(NSMutableURLRequest *)request
  ...
  [request setValue:[[QNUserAgent sharedInstance] description]
  ...
  QNInternalProgressBlock progressBlock2 =
      ^(long long totalBytesWritten, long long totalBytesExpectedToWrite) {
        progressBlock(totalBytesWritten, totalBytesExpectedToWrite);
      };
  __block QNProgessDelegate *delegate =
      [[QNProgessDelegate alloc] initWithProgress:progressBlock2];

  NSURLSessionUploadTask *uploadTask = [_httpManager
      uploadTaskWithRequest:request
          ...
          completionHandler:^(NSURLResponse *response, id responseObject,
            ...
            if (delegate.progress != nil) {
              [delegate.progress removeObserver:delegate
                                     forKeyPath:@"fractionCompleted"
                                        context:(__bridge void *)(delegate)];
              delegate.progress = nil;
            }
            if (_converter != nil && _noProxy &&
                (index + 1 < ips.count || times > 0) &&
                needRetry(httpResponse, error)) {
              [self sendRequest2:request
                  withCompleteBlock:completeBlock
                  withProgressBlock:progressBlock
                    withCancelBlock:cancelBlock
                        withIpArray:ips
                          withIndex:index + 1
                         withDomain:domain
                     withRetryTimes:times - 1
                      withStartTime:startTime];
              return;
            }
            if (error == nil) {
              info = [QNSessionManager buildResponseInfo:httpResponse
                                               withError:nil
                                            withDuration:duration
                                            withResponse:data
                                                withHost:domain
