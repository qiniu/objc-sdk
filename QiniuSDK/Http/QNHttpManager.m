...
- (void)sendRequest2:(NSMutableURLRequest *)request
  ...
  AFHTTPRequestOperation *operation =
      [_httpManager HTTPRequestOperationWithRequest:request
          success:^(AFHTTPRequestOperation *operation, id responseObject) {
            double duration = [[NSDate date] timeIntervalSinceDate:startTime];
            QNResponseInfo *info =
                [QNHttpManager buildResponseInfo:operation
                                       withError:nil
                                    withDuration:duration
                                    withResponse:operation.responseData
                                          withIp:ip];
          ...
          failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            if (_converter != nil && (index + 1 < ips.count || times > 0) &&
                needRetry(operation, error)) {
      ...
      withProgressBlock:progressBlock
        withCancelBlock:cancelBlock
            withIpArray:ips
              withIndex:0
             withDomain:domain
         withRetryTimes:kQNRetryConnectTimes
          withStartTime:startTime];
}
