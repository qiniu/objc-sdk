...
+ (NSString *)pathJoin:(NSString *)key path:(NSString *)path {
...
+ (instancetype)fileRecorderWithFolder:(NSString *)directory
  ...
  return [QNFileRecorder fileRecorderWithFolder:directory
                                      encodeKey:false
                                          error:perror];
}
...
- (NSError *)set:(NSString *)key data:(NSData *)value {
  ...
  [value writeToFile:[self pathOfKey:key]
...
- (NSError *)del:(NSString *)key {
  ...
  [[NSFileManager defaultManager] removeItemAtPath:[self pathOfKey:key]
                                             error:&error];
  return error;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p, dir: %@>",
